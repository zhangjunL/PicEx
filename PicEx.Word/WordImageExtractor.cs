...
namespace PicEx.Word
{
    public class WordImageExtractor
    {
        public static void ExtractImagesFromWord(string wordFilePath, string outputFolderPath)
        {
            ...
            // 将原文件复制到临时文件夹
            string tempExcelFilePath = Path.Combine(tempFolderPath, Path.GetFileName(wordFilePath));
            File.Copy(wordFilePath, tempExcelFilePath);
            ...
            // 获取包含图片的文件夹路径
            string imagesFolderPath = Path.Combine(tempFolderPath, "word", "media");
